...
///
/// Unknown values fall back to cyan and log a warning. Use
/// [`parse_color_checked`] when an invalid value should be rejected instead.
pub fn parse_color(s: &str) -> ratatui::style::Color {
    parse_color_checked(s).unwrap_or_else(|| {
        warn!(color = s.trim(), "unknown color, defaulting to cyan");
        ratatui::style::Color::Cyan
    })
}

/// Parse a color string into a ratatui Color, returning `None` for values that
/// are not a recognized hex, named, rgb, or reset color.
///
/// Supports: hex (#rrggbb, #rgb), named colors, rgb(r,g,b), and reset aliases.
