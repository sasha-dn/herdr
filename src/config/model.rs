...
pub struct KeysConfig {
    ...
    /// Search the focused pane scrollback. Default: "prefix+/".
    pub find: BindingConfig,
...
impl<'de> Deserialize<'de> for KeysConfig {
        ...
        D: Deserializer<'de>,
    {
        ...
        apply_field!(previous_pane);
        apply_field!(next_pane);
...
impl KeysConfig {
    ...
    pub(crate) fn local_profile(&self, keybinds: &Keybinds) -> KeysConfigOverlay {
        ...
        copy_effective_action_field!(previous_pane, keybinds.previous_pane);
        copy_effective_action_field!(next_pane, keybinds.next_pane);
        ...
        copy_effective_action_field!(find, keybinds.find);
...
"#;
        ...
        let toml = r#"
[ui]
agent_panel_sort = "manual"
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.ui.agent_panel_sort, AgentPanelSortConfig::Manual);
        assert_eq!(config.ui.agent_panel_sort.as_str(), "manual");
...
"#;
    ...
    #[test]
    fn removed_config_keys_are_tolerated() {
        // Older configs may still carry the removed `[ui.agent_panel]` row
        // template and the removed `[experimental].hide_tabs_with_agents`
        // key. Unknown fields must be ignored so those configs keep loading.
        let toml = r#"
[ui.agent_panel]
rows = [
  " {icon} {tab}",
  " {space:darkgrey} { · status}",
]

[experimental]
hide_tabs_with_agents = true
"#;
        let config: Config = toml::from_str(toml).unwrap();
        // Unrelated agent-panel config still parses to its default.
        assert_eq!(config.ui.agent_panel_sort, AgentPanelSortConfig::Spaces);
    }
