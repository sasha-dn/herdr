...
herdr agent children <target> [--recursive] [--json]
herdr agent read <target> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi]
herdr agent send <target> <text>
herdr agent rename <target> <name>|--clear
herdr agent set-parent <target> <parent>
herdr agent focus <target>
herdr agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]
herdr agent attach <target> [--takeover]
herdr agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--parent TARGET] [--split right|down] [--env KEY=VALUE] [--focus|--no-focus] -- <argv...>
...
`agent start --parent <target>` spawns the new agent as a split of an existing agent pane (resolved with the same target rules) and records it as that agent's parent. The child then nests under its parent in the agents panel across every sort mode. `--parent` cannot be combined with `--workspace` or `--tab`, and errors if the target does not resolve to an agent pane.

`agent set-parent <target> <parent>` assigns or changes the parent of an already-running agent. Both targets resolve with the same rules and must be agent panes. It only assigns or changes the link; it never clears it. The command rejects a target that is its own parent and any assignment that would create a cycle (a parent that is the child itself or one of its descendants). The change takes effect immediately, so the child re-nests under its new parent on the next agents-panel render, and the link persists across restart.

`agent children <target>` lists the child agents nested under an agent in the agents-panel tree. The target resolves with the same rules and must be an agent pane. By default it prints one public pane id per direct child, one per line, so it drops straight into shell loops; add `--recursive` to include the whole descendant subtree in tree (preorder) order, and `--json` to emit the full agent records instead of bare pane ids.
