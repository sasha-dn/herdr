...
`previous_pane` and `next_pane` cycle focus through the sidebar panes section, one entry per non-agent pane across all spaces. Focusing an entry switches to that pane's space and tab, exactly like clicking the row. Cycling wraps around and skips line-split dividers. They are unset by default; bind them with a modified chord such as `ctrl+shift+up` and `ctrl+shift+down`.
