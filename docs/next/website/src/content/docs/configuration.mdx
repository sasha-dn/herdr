...
| Field | Value |
| --- | --- |
| `{icon}` | Status icon or spinner. Keeps its state color. |
...
A field reference can carry a literal prefix and an optional style role:
...
- `{field:role}` styles the field with a named role: `name` (primary label), `status` (state color), or `muted` (dim secondary text). Each field has a sensible default role. Roles resolve against the active theme, so custom rows stay consistent with your palette.
